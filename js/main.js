...
function createMap() {
    ...
    promises.push(d3.json("data/crimeNews.geojson"));
...
function addLine(props) {
    ...
    d3.selectAll(".dot")
        .remove()
    console.log("type is: "+crimeType)
    var data = []
    if(crimeType==""){
        console.log("wehriuwxnhrounehwxorwiunhexfoiunwehxo")
    var dataset = [
    ...
    const newsCircle = overlay.selectAll('circle')
        ...
        .on('click', function (event, d) {
            if (value == 1) {
                ...
                //showPopup(d, left, top)
                var labelAttribute = d.properties.Intro;
                ///////////////////////////////////////////////////////////////
                // for (let i = 38; i < labelAttribute.length; i++) {
                //     console.log(labelAttribute[i-2])
                //     if (i % 38 == 0 && (labelAttribute[i-2]!="-" || labelAttribute[i-2]!=" ")) {
                //         var newStr = labelAttribute.substring(0, i - 1) + "-" + labelAttribute.substring(i - 1, labelAttribute.length); 
                //     }
                //     else{
                //         var newStr = labelAttribute.substring(0, i - 1) + "-" + labelAttribute.substring(i - 1, labelAttribute.length);
                //     }
                //     labelAttribute = newStr;
        ...
        function update() {
            d3.selectAll("#newsCircle")
            .attr("cx", function (d) { console.log("dddd: "+d.geometry.coordinates);return map.latLngToLayerPoint([d.geometry.coordinates[1], d.geometry.coordinates[0]]).x })
            .attr("cy", function (d) { return map.latLngToLayerPoint([d.geometry.coordinates[1], d.geometry.coordinates[0]]).y })
        }
